...
class NearestNounResolver:
    ...
    def resolve(self, sentence: str, full_text: str, pronoun_info: Dict[str, Any],
        ...
        # 计算代词在完整文本中的位置
        # 注意：这里简化处理，实际需要考虑句子在文本中的位置
        # 对于游记，我们主要在句子内和前几句中查找

        # 策略1：在句子内查找最近的名词
        sentence_before = sentence[:position_in_sentence]
        candidates_in_sentence = self._extract_nouns_from_text(sentence_before)

        # 策略2：优先匹配POI词典中的景点
        poi_candidates = [c for c in candidates_in_sentence if c['entity'] in self.poi_dicts]
        if poi_candidates:
            best_match = poi_candidates[0]
            return {
                "pronoun": pronoun,
                "antecedent": best_match['entity'],
                "resolution_method": "nearest_poi_in_sentence",
                "confidence": "high",
                "reason": f"在句子中找到最近的POI景点: {best_match['entity']}"
            }

        # 策略3：如果没有POI，使用最近的名词
        if candidates_in_sentence:
            # 过滤明显的非实体词
            filtered = [c for c in candidates_in_sentence
            ...
            if filtered:
                ...
                return {
                    "pronoun": pronoun,
                    "antecedent": best_match['entity'],
                    "resolution_method": "nearest_noun_in_sentence",
                    "confidence": "medium",
                    "reason": f"在句子中找到最近的名词: {best_match['entity']}"
                }
    ...
    for sentence_data in all_sentences:
        ...
        for pronoun in sentence_data['pronouns']:
            result = resolver.resolve(
                ...
                sentence_data.get('full_text', sentence_data['sentence']),
                pronoun,
