...
def visualize_gugong_hierarchy(hierarchy: Dict, output_path: str):
    ...
    # 按区域分组
    zones = {
        '外朝（前朝）': ['午门', '太和门', '太和殿', '中和殿', '保和殿', '文华殿', '武英殿'],
        '内廷（后寝）': ['乾清宫', '交泰殿', '坤宁宫', '养心殿', '西六宫区', '东六宫区'],
...
def main():
    ...
    # 创建输出目录
    for dir_name in ['route_hierarchy', 'route_comparison']:
    ...
    visitor_data = load_visitor_data('entity_results.json')
    ...
    for idx, row in df.iterrows():
        ...
        # 保存层级数据
        hierarchy_file = f'route_hierarchy/{scenic_spot}_hierarchy.json'
    ...
    for scenic_spot in spot_en_map.keys():
        if scenic_spot in hierarchy_data:
            ...
            output_path = f'route_hierarchy/{scenic_spot}_hierarchy.png'
            visualize_time_hierarchy(hierarchy_data[scenic_spot], scenic_spot, output_path)

    # 对比分析图
    print("  生成对比分析图...")
    visualize_comparison(full_report, 'route_comparison/comparison_charts.png')
    ...
    print("=" * 60)
    print("\n输出文件:")
    print("  - route_hierarchy/*.json  : 层级结构数据")
    print("  - route_hierarchy/*.png  : 层级结构可视化")
    print("  - route_comparison/comparison_report.json : 对比分析报告")
    print("  - route_comparison/comparison_charts.png  : 对比分析图表")
    print()
