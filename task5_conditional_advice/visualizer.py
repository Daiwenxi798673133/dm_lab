...
class ConditionalAdviceVisualizer:
    ...
    def plot_visitor_comparison(self, output_dir: str):
        ...
        preferred_types = ['family', 'elderly', 'couple', 'experienced']
        main_types = [t for t in preferred_types if t in by_type]

        ranked_types = sorted(
            [(k, v.get('count', 0)) for k, v in by_type.items() if k not in main_types and k != 'general'],
            key=lambda x: x[1],
            reverse=True,
        )
        main_types.extend([item[0] for item in ranked_types[:max(0, 4 - len(main_types))]])
        ...
        for idx in range(len(main_types), 4):
            axes[idx].axis('off')
    ...
    def plot_scenic_spot_comparison(self, output_dir: str):
        ...
        if not by_spot or len(by_spot) < 2:
            print("  跳过景区对比图（数据不足）")
            return

        spots = list(by_spot.keys())
        condition_types = set()
        ...
        for spot in spots:
            row = []
            cond_types = by_spot[spot].get('condition_types', {})
            for cond_type in condition_types:
                row.append(cond_types.get(cond_type, 0))
